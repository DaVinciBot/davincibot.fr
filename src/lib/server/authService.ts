...
import type { AppSession, AppUser } from './sso';

export type ResolveResult =
	| { status: 'ok'; session: AppSession; user: AppUser }
	| { status: 'invalid' }
	| { status: 'unavailable' };
...
const parsePayload = (payload: unknown): { session: AppSession; user: AppUser } | null => {
	...
	return {
		...
		user: {
			...
			app_metadata: (isRecord(u.app_metadata) ? u.app_metadata : {}) as AppUser['app_metadata'],
			user_metadata: (isRecord(u.user_metadata) ? u.user_metadata : {}) as AppUser['user_metadata']
		}
	};
};
